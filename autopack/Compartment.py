...
#                   r=False
    ...
    def BuildGrid_kevin(self, env, display = True, superFine = False):
        """
        ...
        # Create surface points. Copied from BuildGrid_jordan
        ...
        # copied from BuildGrid_jordan
        distances = env.grid.distToClosestSurf
        idarray = env.grid.gridPtId
        diag = env.grid.diag
        
        spacing = env.grid.diag # Added. Not sure if this works.
        gridSpacing = spacing
        radius = gridSpacing

        # Make a copy of faces, vertices, and vnormals.
        # BuildGrid_Jordan does not make copies of these...hm....
        faces = self.faces[:]
        vertices = self.vertices[:]
        vnormals = self.vnormals[:]
        
        # Get the number of the surface we're looking at.
        number = self.number

        # Get the bounding box for our polyhedron
        corners = env.grid.boundingBox

        # Grid initialization referenced from getSurfaceInnerPointsJordan()
        grid = env.grid
        # grid.boundingBox = env.grid.boundingBox
        ...
        for g in gridPoints:
            if g.representsPolyhedron == True:
                ...
                if g.isOutside == None:
                    g.isOutside = True
                if g.isOutside == False:
                    idarray.itemset(g.index, -number)
        ...
        self.insidePoints = insidePoints
        self.outsidePoints = outsidePoints
        self.surfacePointsCoords = [g.globalCoord for g in gridPoints if g.representsPolyhedron == True]
